...
function [w,lambda,metaInfo] = L1_sgd(TrainFea, TrainGnd, ATK_options, SGD_options)

% Learning with Average Top-K Loss: for linear regression, individual
% loss is absolute loss, linear regression, i.e., y = w^T*x
%
% Input
%   TrainGnd (n*1): 
...
while i_pass <= NumPass
    for i = 1 : Ntrain
        ...
        if loss_i > lambda
            factor = -1; if temp_i > 0, factor = 1; end
...
%calculate loss
temp1 = TrainFea * w - TrainGnd;
loss =  abs(temp1) - lambda; 
