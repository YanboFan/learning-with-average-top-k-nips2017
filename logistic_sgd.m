...
function [w,lambda] = logistic_sgd(TrainFea, TrainGnd, ATK_options, SGD_options)

% Learning with Average Top-K Loss: for binary classification, individual
% loss is logistic loss, linear classifier, i.e., y = w^T*x
%
% Input
%   TrainGnd (n*1): 
...
%initialize lambda
loss = 1 + exp(min(-TrainGnd .* (TrainFea * w),100));
loss = log(loss);  temp1 = sort(loss,'descend');
...
max_lambda = log(2); lambda = min(lambda, max_lambda);
...
while i_pass <= NumPass
    for i = 1 : Ntrain
        ...
        %preparing
        step = a / sqrt(t);
        temp_i = exp(min(tempData(i,:)*w,100)); %
        loss_i = log(1 + temp_i); %

        if loss_i > lambda
            factor = temp_i / (temp_i+1);
            w = w - step*(w + ATK_c*factor*tempData(i,:)');
